				...
				if($q_edit){
					...
					if(isset($_FILES['foto']) && $_FILES['foto']['name'][0] != ''){ 
					 	foreach ($_FILES['foto']['name'] as $key => $value) {
							...
							$titulo_final = "blog_".$id.'_'.time()."_".rand().md5(time()).'.'.$extensao;
							 
							if (move_uploaded_file($_FILES['foto']['tmp_name'][$key], $_UP['pasta'] . $titulo_final)) {
								$foto = $titulo_final;  
								if($foto != '')mysqli_query($conn,"INSERT INTO sp_servicos_fotos (servicoID,foto) VALUES ('$id','$foto')");
							} else {   ?> 
								<center>
									<h5 style="color:rgb(237,50,55);">Erro ao salvar!<br>Tente novamente mais tarde.</h5><br>
									<a class="close button" href="index.php?go=servicos&edit=<?php echo $id ?>" aria-label=" ">Voltar</a>
						...
						<form data-abide action="?go=servicos&edit=<?php echo $id ?>&validaedit" method="POST" enctype="multipart/form-data"> 
							...
							<div class="small-12 large-4 columns">
								...
								<fieldset><!-- fotos cadastradas -->
									...
									<?php  
										$sqlFotos = "SELECT * FROM sp_servicos_fotos WHERE servicoID = '$id'";
												...
												<div class="small-6 columns end" style="margin-bottom: 10px;">
													<div style="position: absolute;">
														...
														<div style="padding:20px;width: 35%;text-align:center;" id="delblog<?php echo $idblogi ?>" class="reveal-modal" data-reveal aria-labelledby="modalTitle" aria-hidden="true" role="dialog">
															...
															<a href="inc/deleteData.php?id=<?php echo $idblogi ?>&tabela=sp_servicos_fotos&volta=servicosEdit&id_volta=<?php echo $id ?>" style="color:white"><button class="excluirS">Excluir</button></a>
