...
<?php 
	...
	if(isset($_GET['go']) && valida_campos($_GET['go'],$conn) == 'gerais'){ 
		if (isset($_GET['validainsert'])) {
			...
			$query = mysqli_query($conn,"INSERT INTO sp_gerais (email,telefone,instagram,facebook,youtube,imagemfundobanner) 
				VALUES ('$email','$telefone','$instagram','$facebook','$youtube','$imagemfundobanner')");
			if($query){
				$id_tarefa = mysqli_insert_id($conn);
				if(isset($_FILES['imagemfundobanner']) && $_FILES['imagemfundobanner']['name'] != '' && $_FILES['imagemfundobanner']['size'] != 0){
					$_UP['pasta'] = 'anexos/';  
					...
					if ($_FILES['imagemfundobanner']['error'] != 0) {?> 
						<center>
							<h5 style="color:rgb(237,50,55);">Erro ao editar!<br><?php echo $_UP['erros'][$_FILES['imagemfundobanner']['error']] ?>.</h5><br>
							<a class="close button" href="index.php?go=gerais">Voltar</a>
						</center>
						<?php 
						exit;   
					} 
					$extensao = strtolower(end(explode('.', $_FILES['imagemfundobanner']['name'])));
					if (array_search($extensao, $_UP['extensoes']) === false) { ?> 
						<center>
							<h5 style="color:rgb(237,50,55);">Erro ao cadastrar!<br>A extensão "<?php echo $extensao ?>" não é permitida.</h5><br>
							<a class="close button" href="index.php?go=gerais" aria-label=" ">Voltar</a>
						</center>
						<?php 
						exit; 
					} 
					$nome_final = 'imagemfundoobras_' . time() . "_" . uniqid().".".$extensao;
					if (move_uploaded_file($_FILES['imagemfundobanner']['tmp_name'], $_UP['pasta'] . $nome_final)) { 
						mysqli_query($conn,"UPDATE sp_gerais SET imagemfundobanner = '$nome_final'");
					} else {   ?> 
						<center>
		...
		<fieldset>
			...
			<form data-abide action="?go=gerais&validainsert" method="POST" enctype="multipart/form-data">
				...
				<div class="small-12 large-3 columns">
					<label>Facebook:
						<input name="facebook" id="facebook" value="<?php echo $rEmrepsa['facebook'] ?>" required="" pattern="[a-zA-Z0-9]+" type="text">
					</label>
					<small class="error">Preencha este campo.</small>
				</div>
				<div class="small-12 large-3 columns">
					<label>Instagram:
						<input name="instagram" id="instagram" value="<?php echo $rEmrepsa['instagram'] ?>" required="" pattern="[a-zA-Z0-9]+" type="text">
					</label>
					<small class="error">Preencha este campo.</small>
				</div>
				<div class="small-12 large-3 columns">
					<label>Youtube:
						<input name="youtube" id="youtube" value="<?php echo $rEmrepsa['youtube'] ?>" required="" pattern="[a-zA-Z0-9]+" type="text">
					</label>
					<small class="error">Preencha este campo.</small>
				</div>
				<div class="small-12 large-3 columns">
					<label>Telefone/Whatsapp:
						<input name="telefone" id="telefone" value="<?php echo $rEmrepsa['telefone'] ?>" required="" onkeypress="mascara(this,mtel)" maxlength="15" pattern="[0-9]+"  type="text">
					</label>
					<small class="error">Preencha este campo.</small>
				</div>
