...
<?php 
	...
	if(isset($_GET['go']) && $_GET['go'] == 'depoimentos'){
		if(!isset($_GET['edit'])){ /*inicio*/
			if (isset($_GET['validainsert'])) {
				...
				if(isset($_FILES['foto']) && $_FILES['foto']['name'] != ''){ 
					...
					if ($_FILES['foto']['error'] != 0) {  ?> 
						<center>
							...
							<a class="close button" href="index.php?go=depoimentos">Voltar</a>
						</center>
						<?php 
						exit;   
					} 
					...
					if (array_search($extensao, $_UP['extensoes']) === false) { ?> 
						<center>
							...
							<a class="close button" href="index.php?go=depoimentos" aria-label=" ">Voltar</a>
						</center>
						<?php 
						exit; 
					} 
					$titulo_final = "depoimentos".$id.'_'.time().uniqid().".".$extensao;
					if (move_uploaded_file($_FILES['foto']['tmp_name'], $_UP['pasta'] . $titulo_final)) { 
						// mysqli_query($conn,"INSERT INTO sp_depoimentos_fotos (id_depoimentos,foto) VALUES ('$id_depoimentos','$titulo_final')"); 
					} else {   ?> 
						<center>
				...
				$sqlEdit = "INSERT INTO sp_depoimentos (titulo,cargo,foto,descricao) VALUES ('$titulo','$cargo','$titulo_final','$descricao')"; 
				$q_edit = mysqli_query($conn,$sqlEdit);
				if($q_edit){			
					include "inc/salvo.php";
					echo "<script>setTimeout('depoimentos()', 400);</script>";
				}
				else{ ?> 
					<center> 
						<h5 id="modalTitle" style="color:rgb(237,50,55);">Erro ao salvar!<br>Tente novamente mais tarde.</h5><br>
						<a class="close button" href="index.php?go=depoimentos" aria-label="Close">Voltar</a>
					</center>
					<?php 
					exit;
				} 
			...
			<fieldset>
				<legend>
					...
					<a href="?go=depoimentos" style="color:#008CBA;">DEPOIMENTOS</a>
				</legend> 
				<form> 
					<div id="tabela">
						<i class="fa fa-search" style="float:left;font-size:20px; margin:4px; color:#ccc; position:absolute;"></i>
						<input type="text" autocomplete="off" placeholder="Buscar depoimentos" id="txtColuna1"  class="input_busca_lote" style="padding-left: 26px!important;"> 
							...
							<tbody>
								<?php 
									$query_orgaos = mysqli_query($conn,"SELECT 
										sp_depoimentos.id,
										sp_depoimentos.foto,
										sp_depoimentos.titulo,
										sp_depoimentos.cargo
										FROM sp_depoimentos 
									");
									$numeroLinhas = mysqli_num_rows($query_orgaos);
									if($numeroLinhas > 0){
										while($rowOrgao = mysqli_fetch_array($query_orgaos,MYSQLI_ASSOC)){ 
											$iddepoimentosi = $rowOrgao['id']; ?>
											<div style="padding:20px;width: 35%;text-align:center;" id="deldepoimentos<?php echo $iddepoimentosi ?>" class="reveal-modal" data-reveal aria-labelledby="modalTitle" aria-hidden="true" role="dialog">
												<h5 id="modalTitle">Tem certeza que deseja excluir esta informação: <br><?php echo $rowOrgao['titulo'] ?> ?</h5>
												<a href="inc/deleteData.php?id=<?php echo $iddepoimentosi ?>&tabela=sp_depoimentos&volta=depoimentos" style="color:white"><button class="excluirS">Excluir</button></a>
												<a onclick="closeprofissional()" style="color:white"><button style='background:#0077bd;'>Cancelar</button></a>
											</div>	 
											<tr>
												<td> 
													<a style="background:#0077bd;" class="div_add_person fa-pencil" href="?go=depoimentos&edit=<?php echo $iddepoimentosi ?>"></a>
													<a class="div_add_person fa fa-times" style=" margin-left:5px;" data-reveal-id="deldepoimentos<?php echo $iddepoimentosi ?>" title="Excluir"></a>  
												</td>  
											...
											<input id="iddepoimentosi" value="<?php echo $iddepoimentosi ?>" disabled type="hidden">	 
											<script type="text/javascript"> function closeprofissional(){ $('#deldepoimentos'+$('#iddepoimentosi').val()).foundation('reveal', 'close'); }; </script>
											<?php 
										} 
									}
									else{
				...
				<div class="iframe_lote">
					...
					<div class="small-12 large-12 columns">
						<fieldset style='min-height:375px;'>
							...
							<form data-abide action="?go=depoimentos&validainsert" method="POST" enctype="multipart/form-data">  
								<div class="small-12 large-12 columns">
									<label>Título:
								...
								<div class="small-12 large-12 columns">
									<label>Depoimento:
										<textarea name="descricao" id="descricao" required pattern="[a-zA-Z0-9]+"  style="min-height: 122px;"></textarea>
									</label>
									<small class="error">Preencha este campo</small>
								</div>
			...
			if (isset($_GET['validaedit'])) {
				...
				$sqlEdit = "UPDATE sp_depoimentos SET cargo='$cargo', titulo = '$titulo', descricao = '$descricao' WHERE id = '$id'";
				...
				if($q_edit){   
					...
					if(isset($_FILES['foto']) && $_FILES['foto']['name'] != '' && $_FILES['foto']['size'] > 0){  
						...
						if ($_FILES['foto']['error'] != 0) {  ?> 
							<center>
								<h5 style="color:rgb(237,50,55);">Erro ao editar!<br><?php echo $_UP['erros'][$_FILES['foto']['error']] ?>.</h5><br>
								<a class="close button" href="index.php?go=depoimentos&edit=<?php echo $id ?>">Voltar</a>
							</center>
							<?php 
							exit;   
						} 
						...
						if (array_search($extensao, $_UP['extensoes']) === false) { ?> 
							<center>
								<h5 style="color:rgb(237,50,55);">Erro ao cadastrar!<br>A extensão "<?php echo $extensao ?>" não é permitida.</h5><br>
								<a class="close button" href="index.php?go=depoimentos&edit=<?php echo $id ?>" aria-label=" ">Voltar</a>
							</center>
							<?php 
							exit; 
						} 
						$titulo_final = "depoimentos".$id.'_'.time().uniqid().".".$extensao;
						if (move_uploaded_file($_FILES['foto']['tmp_name'], $_UP['pasta'] . $titulo_final)) { 
							$sqlfoto = "UPDATE sp_depoimentos SET foto='$titulo_final' WHERE id = '$id'";
							// exit;
					...
					<center>
						<h5 id="modalTitle" style="color:rgb(237,50,55);">Erro ao salvar!<br>Tente novamente mais tarde.</h5><br>
						<a class="close button" href="index.php?go=depoimentos&edit=<?php echo $id ?>" aria-label="Close">Voltar</a>
					</center>
					<?php 
					exit;
				}
			...
			<fieldset>
				<?php 
				$id = valida_campos($_GET['edit'],$conn);
				$qorgaogestor = mysqli_query($conn,"SELECT * FROM sp_depoimentos WHERE id = ".$id);
				...
				<form data-abide action="?go=depoimentos&edit=<?php echo $id ?>&validaedit" method="POST" enctype="multipart/form-data"> 
					...
					<div class="small-12 large-3 columns">
						<label>Cargo:
							<input name="cargo" id="cargo" value="<?php echo $rowOrgaoGestor['cargo'] ?>" required pattern="[a-zA-Z0-9]+" type="text">
						</label>
						<small class="error">Preencha este campo</small>
					</div> 
					<div class="small-12 large-12 columns">
						<label>Depoimento:
							<textarea name="descricao" id="descricao" required pattern="[a-zA-Z0-9]+" style="min-height: 122px;"><?php echo $rowOrgaoGestor['descricao'] ?></textarea>
						</label>
						<small class="error">Preencha este campo</small>
					</div>
					<div class="small-12 columns"><p></p></div>
