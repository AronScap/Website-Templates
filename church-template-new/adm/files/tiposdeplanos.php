<?php 
	...
	if(isset($_GET['go']) && $_GET['go'] == 'tiposdeplanos'){
		if(!isset($_GET['edit'])){ /*inicio*/
			if (isset($_GET['validainsert'])) { 
				...
				$q_profissionai = mysqli_query($conn,"SELECT nome FROM sp_tiposdeplanos WHERE nome LIKE '".$nome."'"); 
				if(!mysqli_num_rows($q_profissionai)){	 
					$sqlEdit = "INSERT INTO sp_tiposdeplanos (nome) VALUES ('$nome')"; 
					$q_edit = mysqli_query($conn,$sqlEdit);
					if($q_edit){				
						include "inc/salvo.php";
						echo "<script>setTimeout('tiposdeplanos()', 400);</script>";
					}
					else{ ?> 
						<center> 
							<h5 id="modalTitle" style="color:rgb(237,50,55);">Erro ao salvar!<br>Tente novamente mais tarde.</h5><br>
							<a class="close button" href="index.php?go=tiposdeplanos" aria-label="Close">Voltar</a>
						</center>
						<?php 
						exit;
					}
			...
			<fieldset>
				<legend>
					...
					<a href="?go=tiposdeplanos" style="color:#008CBA;">TIPOS DE PLANOS</a>
				</legend> 
				<form> 
					<!-- <div id="tabela">
						<i class="fa fa-search" style="float:left;font-size:20px; margin:4px; color:#ccc; position:absolute;"></i>
						<input type="text" autocomplete="off" placeholder="Buscar tiposdeplanos" id="txtColuna1"  class="input_busca_lote" style="padding-left: 26px!important;"> 
					...
					<div id="divConteudo"> 						
						<table>
							...
							<tbody>
								<?php 
									$query_orgaos = mysqli_query($conn,"SELECT * FROM sp_tiposdeplanos ORDER BY nome ASC LIMIT 200");
									$numeroLinhas = mysqli_num_rows($query_orgaos);
									if($numeroLinhas > 0){
										while($rowOrgao = mysqli_fetch_array($query_orgaos,MYSQLI_ASSOC)){ 
											$idcati = $rowOrgao['id']; ?>
											<div style="padding:20px;width: 35%;text-align:center;" id="deltiposdeplanos<?php echo $idcati ?>" class="reveal-modal" data-reveal aria-labelledby="modalTitle" aria-hidden="true" role="dialog">
												<h5 id="modalTitle">Tem certeza que deseja excluir esta informação: <br><?php echo $rowOrgao['nome'] ?> ?</h5>
												<a href="inc/deleteData.php?id=<?php echo $idcati ?>&tabela=sp_tiposdeplanos&volta=tiposdeplanos" style="color:white"><button class="excluirS">Excluir</button></a>
												<a class="closeprofissional" style="color:white"><button style='background:#0077bd;'>Cancelar</button></a>
											</div>	 
											<tr>
												<td>
													<a style="background:#0077bd" class="div_add_person fa-pencil" href="?go=tiposdeplanos&edit=<?php echo $idcati ?>"></a>
													<?php  
														$apaga = false;
														...
														if ($apaga) { 	?>
															...
															<a class="div_add_person fa fa-times" style=" margin-left:5px;" data-reveal-id="deltiposdeplanos<?php echo $idcati ?>" title="Excluir"></a>											 
															<?php
														}
													?> 											 
												</td>   
											...
											<script type="text/javascript">
												$('a.closeprofissional').on('click', function() {
												  $('#deltiposdeplanos'+$('#idcati').val()).foundation('reveal', 'close');
												});
											</script>				
											<?php 
										} 
					...
					<div class="iframe_lote">
						...
						<div class="small-12 large-12 columns">
							<fieldset style='min-height:375px;'>
								...
								<form data-abide action="?go=tiposdeplanos&validainsert" method="POST" enctype="multipart/form-data"> 
			...
			if (isset($_GET['validaedit'])) {  	 
				...
				// $descricao = valida_campos($_POST['descricao'],$conn);  
				$q_profissionai = mysqli_query($conn,"SELECT nome FROM sp_tiposdeplanos WHERE id <> '$id' AND nome LIKE '".$nome."'");
				if(!mysqli_num_rows($q_profissionai)){ 
					$q_edit = mysqli_query($conn,$sqlEdit);
					$sqlEdit = "UPDATE sp_tiposdeplanos SET nome = '$nome' WHERE id = ".$id; 
					$q_edit = mysqli_query($conn,$sqlEdit);
					if($q_edit){				
						include "inc/salvo.php";
						echo "<script>setTimeout('tiposdeplanos()', 400);</script>";
			...
			<fieldset>
				<?php 
				$id = valida_campos($_GET['edit'],$conn);
				$qorgaogestor = mysqli_query($conn,"SELECT * FROM sp_tiposdeplanos WHERE id = ".$id);
				...
				<form data-abide action="?go=tiposdeplanos&edit=<?php echo $id ?>&validaedit" method="POST"> 
					<div class="small-12 large-6 columns">
						<label>Nome: <small>Obrigatório</small>
